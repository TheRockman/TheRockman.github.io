var app = angular.module("myApp", ['ngAnimate']); app.controller("mainCtrl", function($scope, $timeout) {
  ...
  function getId() {
    for (var i = 0; i < $scope.cubes.length; i++) {
      ...
      if ($scope.cubes[i].id === 44) {
        $scope.cubes[i].state = 'player2';
      }
      if ($scope.cubes[i].id === 46) {
  ...
  $scope.nbCubes = $scope.cubes.length;
  $scope.cubeSize = $scope.nbCubes*2;
  $scope.cubeMargin = $scope.nbCubes/2;
  $scope.perspective = '200em';
  ...
  function buildCubes() {
      var persSize = $scope.nbCubes * ($scope.cubeSize + $scope.cubeMargin*2);
  
      angular.element(document.querySelector('#perspective'))
          .css('width', $scope.persSize+'px')
          .css('height', $scope.persSize+'px')
          .css('margin-left', (-$scope.persSize/2)+'px')
          .css('margin-top', (-$scope.persSize/2)+'px')
          .css('-webkit-perspective', $scope.perspective)
          .css('perspective', $scope.perspective);
  }  
  buildCubes();
