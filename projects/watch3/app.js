var app = angular.module("myApp", ['ngTouch']); app.controller("mainCtrl", function($scope, $timeout) {
  ...
  $scope.setCurrentDebate = function(debate) {
    if (!debate) {
      ...
      var clone = JSON.parse(JSON.stringify(debate));

      clone.pitch.desc = clone.pitch.desc.replaceAll("*", space);
      clone.pitch.desc = clone.pitch.desc.split(" ");
      $scope.currentDebate = clone;
    }
  };
  ...
  $scope.setCurrentArgument = function(argument) {
    if (!argument) {
      ...
      var clone = JSON.parse(JSON.stringify($scope.currentDebate.arguments[argument]))

      clone.desc = clone.desc.replaceAll("*", space);
      clone.desc = clone.desc.split(" ");

      clone.descApproved = clone.descApproved.replaceAll("*", space);
      clone.descApproved = clone.descApproved.split(" ");

      clone.descRejected = clone.descRejected.replaceAll("*", space);
      clone.descRejected = clone.descRejected.split(" ");

      $scope.currentArgument = clone;
    }
  };
